...
def public_folder(path : String)
  Grip.config.public_folder = path
end
...
def serve_static(status : (Bool | Hash))
  Grip.config.serve_static = status
end
...
def send_file(env : HTTP::Server::Context, path : String, mime_type : String? = nil, *, filename : String? = nil, disposition : String? = nil)
  config = Grip.config.serve_static
  ...
  Grip.config.static_headers.try(&.call(env.response, file_path, filestat))
  ...
  File.open(file_path) do |file|
    ...
    condition = config.is_a?(Hash) && config["gzip"]? == true && filesize > minsize && Grip::Utils.zip_types(file_path)
...
# Adds headers to `Grip::StaticFileHandler`. This is especially useful for `CORS`.
#
# ```
...
def static_headers(&headers : HTTP::Server::Response, String, File::Info -> Void)
  Grip.config.static_headers = headers
end
